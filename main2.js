...
buttonNext.addEventListener("click", () => {
  //le pasamos dos argumento el click y toda la funcion que esta abajo
  numerodePokemon += 1;
  cargaDePokemon(numerodePokemon);
});
buttonAtrapar.addEventListener("click", () => {
  const storage = localStorage.getItem("lista");
  var img = pokemonImagen.src; //llamar a la imagen del pokemon
  var nombre = pokemonNombre.innerHTML; // llamos al nombre del pokemon
  let lista = []; //declaro array sin nada
  let myObj = new Object(); // declaro un objeto sin nada { }
  myObj["nombre"] = nombre;
  myObj["src"] = img;
  lista.push(myObj); // para introducir el objeto
